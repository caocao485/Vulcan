...
public class CallByValueVisitor implements AstVisitor<JamVal> {
    ...
    private JamVal forClosureVal(final ClosureVal rator, final App a) {
        ...
        for (int i = 0; i < params.length; i++) {
            frame.put(params[i], null);
        }

        for (int i = 0; i < params.length; i++) {
            frame.put(params[i], args[i].accept(this));
        }
        final Env<JamVal> env0 = env;
        env = Env.extendEnv(frame, rator.getEnv());
        final JamVal result = rator.getBody().accept(this);
        env = env0;
        return result;
    }
    ...
    public JamVal forLet(final Let letAst) {
        ...
        for (final Def def : letAst.getDefs()) {
            frame.put(def.lhs(), def.rhs().accept(this));
        }
        final Env<JamVal> env0 = env;
        env = Env.extendEnv(frame, env);
        final JamVal result = letAst.getBody().accept(this);
        env = env0;
        return result;
    }
}
